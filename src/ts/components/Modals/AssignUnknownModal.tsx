...
import { close_modal, house_assign_unknown } from "ts/state/actions";
import { root_state_t } from "ts/state/reducers";
import TreacheryCard from "ts/components/TreacheryCard";

const AssignUnknownModal: React.FC<{ house: house_name_t }> = ({ house }) => {
  ...
  return (
    <Modal header="Assign a card to this unknown" close={() => dispatch(close_modal())} isFull>
            ...
            <div className="column is-half is-one-quarter-widescreen" key={"card-" + index}>
              <TreacheryCard
                ...
                onClick={() => {
                  ...
                  dispatch(house_assign_unknown(house, unknown_index, possible.card.id));
...
export default AssignUnknownModal;
